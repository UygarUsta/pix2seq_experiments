...
train_transform = A.Compose([
    A.HorizontalFlip(p=0.5),
        # 2. Scale & Crop (Orijinal Pix2Seq LSJ mantığı)
        ...
        A.RandomCrop(height=IMG_H, width=IMG_W, p=0.5),
        A.Resize(IMG_H, IMG_W),
        # 3. Hafif Renk/Kontrast Değişimi (Aşırı gürültüden kaçınarak)
        A.ColorJitter(
...
if __name__ == "__main__":
    ...
    # CNN omurgası düşük lr ile fine-tune, geri kalan her şey tam lr ile sıfırdan
    ...
    # Güvenlik ağı: mimariyi değiştirince unutulan parametre olursa burada patlasın
    n_in_opt = sum(len(g["params"]) for g in optimizer.param_groups)
    n_total  = sum(1 for p in model.parameters() if p.requires_grad)
    assert n_in_opt == n_total, f"{n_total - n_in_opt} parametre optimizer dışında kaldı!"

    total_steps = len(train_dataloader) * EPOCHS
    scheduler = OneCycleLR(
        optimizer,
        max_lr=[3e-5, LEARNING_RATE],      # uzunluk param grup sayısıyla aynı olmalı
    ...
    for epoch in epoch_bar:
        ...
        for images, decoder_input, decoder_target in train_bar:
            ...
            tgt_pad_mask = (decoder_input == PAD_TOKEN)

            optimizer.zero_grad()
            with torch.autocast(device_type=device.type, dtype=amp_dtype, enabled=amp_enabled):
                logits = model(images, decoder_input, tgt_key_padding_mask=tgt_pad_mask)
