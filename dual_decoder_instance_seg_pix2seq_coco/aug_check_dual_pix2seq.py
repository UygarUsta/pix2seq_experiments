"""
aug_check_dual_pix2seq.py
=========================
Sequence-augmentation görsel kontrolü (COCO sürümü).
...
LSJ sonrası kırpma, sıralama, noise'un kuyruğa eklenmesi ve poligon eşleşmesi
hepsi burada doğrulanmış olur.
...
from dual_decoder_pix2seq import (
    CocoDualPix2SeqDataset, dequantize,
    TRAIN_ANN, TRAIN_IMG_DIR, VAL_ANN, VAL_IMG_DIR,
    IMG_SIZE, ID_TO_LABEL, NUM_BINS, MAX_OBJECTS,
    NOISE_CLASS_TOKEN, PAD_TOKEN,
...
def decode_box_sequence(box_in):
    ...
    for s in range(MAX_OBJECTS):
        ...
        objs.append({
            "slot": s,
            "cls_token": chunk[4],
            "is_noise": chunk[4] == NOISE_CLASS_TOKEN,
            "box": [dequantize(chunk[0], IMG_SIZE[1], NUM_BINS),
...
def render_sample(img_tensor, box_in, mask_in, n_real, noise_limit=25, poly_alpha=0.28):
    ...
    for o in objs:                          # önce poligon dolgusu
        if o["is_noise"]:
            continue
    ...
    n_noise = drawn_noise = 0
    for o in objs:
        x0, y0, x1, y1 = o["box"]
        if o["is_noise"]:
            n_noise += 1
            # MAX_OBJECTS=100'de kuyruk ~90 noise kutusu; hepsini çizmek görüntüyü
            # okunamaz hâle getirir, ilk noise_limit tanesi yeter.
            if drawn_noise >= noise_limit:
                continue
            drawn_noise += 1
            d.rectangle([x0, y0, x1, y1], outline=C_NOISE, width=1)
            continue

        tag = ID_TO_LABEL.get(o["cls_token"] - NUM_BINS, "?")
        d.rectangle([x0, y0, x1, y1], outline=C_GT, width=3)
        label = f"{o['slot']}:{tag}"
        tw = 7 * len(label) + 8
        ty = max(0, y0 - 15)
        d.rectangle([x0, ty, x0 + tw, ty + 15], fill=C_GT)
    ...
    hd.rectangle([110, 8, 122, 20], outline=C_NOISE, width=3)
    hd.text((128, 9), f"noise: {n_noise} (çizilen {drawn_noise})", fill=C_NOISE)
    hd.text((330, 9), f"slot: {len(objs)}/{MAX_OBJECTS}", fill=(200, 200, 200))
    return out, n_noise
...
def verify_sample(box_in, box_tgt, mask_in, n_real):
    ...
    if noise:
        assert min(o["slot"] for o in noise) > max((o["slot"] for o in real), default=-1), \
            "noise gerçek nesnelerin arasına girmiş (kuyrukta olmalı)"
    for o in noise:
        tgt = box_tgt[o["slot"] * BOX_TOKENS_PER_OBJ:
                      (o["slot"] + 1) * BOX_TOKENS_PER_OBJ].tolist()
        assert tgt == [PAD_TOKEN] * 4 + [NOISE_CLASS_TOKEN], f"noise hedefi bozuk: {tgt}"
        assert all(t == PAD_TOKEN for t in mask_in[o["slot"]].tolist()), \
            "noise'a maske dizisi verilmiş"
    for o in real:
        tgt = box_tgt[o["slot"] * BOX_TOKENS_PER_OBJ:
                      (o["slot"] + 1) * BOX_TOKENS_PER_OBJ].tolist()
...
def main():
    ...
    ap.add_argument("--ann", default=None, help="annotation json (varsayılan: train/val)")
    ...
    ap.add_argument("--noise-limit", type=int, default=25)
    ...
    ann = args.ann or (VAL_ANN if args.val else TRAIN_ANN)
    img_dir = args.img_dir or (VAL_IMG_DIR if args.val else TRAIN_IMG_DIR)

    ds = CocoDualPix2SeqDataset(
        ann, img_dir,
    ...
    tot_real = tot_noise = empty_imgs = 0
    for i, idx in enumerate(idxs):
        img_t, box_in, box_tgt, mask_in, _mask_tgt, n_real = ds[idx]

        if not args.no_verify:
            verify_sample(box_in, box_tgt, mask_in, n_real)

        canvas, n_noise = render_sample(img_t, box_in, mask_in, int(n_real),
                                        noise_limit=args.noise_limit)
        stem = os.path.splitext(str(ds.names[idx]))[0]
        canvas.save(os.path.join(args.out,
                                 f"{i:03d}_{stem}_gt{int(n_real)}_noise{n_noise}.jpg"),
                    quality=92)
    ...
    if not args.val and tot_real / max(n, 1) < 3.0:
        print("  [uyarı] LSJ sonrası ortalama GT düşük. SCALE_RANGE üst sınırını "
              "veya MIN_AREA_KEEP'i gözden geçir.")
