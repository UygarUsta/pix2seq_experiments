...
def seq_to_quads(tokens, tok_scores=None):
    ...
    for i in range(0, n, TOKENS_PER_OBJ):
        ...
        scores.append(1.0 if tok_scores is None
                      else float(np.prod(tok_scores[i:i + TOKENS_PER_OBJ])))
...
def ar_decode_batch(model, images, max_seq_len=MAX_SEQ_LEN, device=None, use_cache=True):
    """Batch greedy decode. Returns (tokens [B,L], per-token probs [B,L])."""
    model.eval()
...
# ─────────────────────────────────────────────────────────────────────────────
# COCO-STYLE AVERAGE PRECISION OVER ARBITRARY IoU
# ─────────────────────────────────────────────────────────────────────────────
...
def evaluate(model, loader, max_seq_len=MAX_SEQ_LEN, device=None,
    ...
    for images, targets in loader:
        ...
        for b in range(images.size(0)):
            ...
            pq, pl, ps = seq_to_quads(toks, scs[b].float().cpu().numpy())

            gt_toks = [t for t in targets[b].tolist()
                       if t not in (BOS_TOKEN, EOS_TOKEN, PAD_TOKEN)]
            gq, gl, _ = seq_to_quads(gt_toks, None)
...
if __name__ == "__main__":
    ...
    from training_pix2seq import Pix2SeqDataset, JSON_DIR, IMG_DIR, val_transform, VAL_SPLIT_PATH
    ...
    val_ds = Pix2SeqDataset(JSON_DIR, IMG_DIR, transform=val_transform)
