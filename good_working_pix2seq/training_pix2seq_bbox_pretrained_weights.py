...
JSON_DIR       = "/mnt/d/Datasets/widerface/train"
IMG_DIR        = "/mnt/d/Datasets/widerface/train"
NUM_BINS = 500 #1000
MAX_OBJECTS = 100
BATCH_SIZE = 16
EPOCHS = 350
...
class Pix2SeqDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.transform is not None and random.random()  < 0.0 : #< 0.3:
            ...
            valid_shapes.sort(key=lambda s: (min(p[1] for p in s["points"]),
                                            min(p[0] for p in s["points"])))
        ...
        for shape in valid_shapes:
            ...
            # clip to image, then reject degenerate / out-of-frame boxes
...
if __name__ == "__main__":
    ...
    sd = torch.load("pix2seq_best_map_bbox_0.1104_cmini_165ep.pth", map_location="cpu")
    ...
    for k in range(20):
        ...
        # for i in range(0, len(toks) - len(toks) % 9, 9):
        #     o = toks[i:i+9]
        #     pts = [((o[j] / (NUM_BINS-1)) * 512, (o[j+1] / (NUM_BINS-1)) * 512)
        #         for j in range(0, 8, 2)]
        #     d.polygon([c for p in pts for c in p], outline="lime", width=3)
        for i in range(0, len(toks) - len(toks) % 5, 5):
            o = toks[i:i+5]
            x0, y0, x1, y1 = [(o[j] / (NUM_BINS - 1)) * 512 for j in range(4)]
    ...
    from bbox_eval_utils_pix2seq_fast import evaluate 
    EVAL_EVERY = 5 
    ...
    for epoch in epoch_bar:
        ...
        if avg_val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), "pix2seq_best_bbox.pth")
            tqdm.write(f"  ✓ Epoch {epoch+1:3d} — new best val loss: {avg_val_loss:.4f} → saved pix2seq_best_bbox.pth")
        ...
        if (epoch + 1) % EVAL_EVERY == 0 or epoch > EPOCHS - 20:
            ...
            if mAP > best_map:
                best_map = mAP
                torch.save(model.state_dict(), "pix2seq_best_map_bbox.pth")
