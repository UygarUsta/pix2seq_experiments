...
#         img_name = item.get("imagePath", self.json_files[idx].replace('.json', '.jpg'))
#         img_path = os.path.join(self.img_dir, img_name)
        
#         try:
#             image = Image.open(img_path).convert("RGB")
#             # 1. Orantıyı bozmadan kareye tamamla (padding)
#             image, _ = pad_to_square(image) 
#             image_np = np.array(image) # Albumentations NumPy array ister
...
#         # # 2. Objeleri Sırala (Yukarıdan Aşağıya)
#         # def sort_key(shape):
...
#         # 3. Noktaları Albumentations formatı için tek bir listeye topla
#         all_points = []
#         valid_shapes = [] # Sadece geçerli objeleri tutacağız
...
#         # 4. Augmentasyonu Uygula (Resim ve noktalar AYNI ANDA değişir/boyutlanır)
...
#         # Albumentations Resize yaptığı için artık hedef boyutumuz IMG_SIZE
#         img_w, img_h = self.img_size[0], self.img_size[1]
        
#         sequence = [BOS_TOKEN]
#         point_idx = 0
        
#         # 5. Güncellenmiş noktaları token'lara çevir
#         # for shape in valid_shapes:
#         #     class_id = LABEL_TO_ID[shape["label"]]
            
#         #     # Her obje için yeni listeden 4 nokta alıyoruz
#         #     for _ in range(4):
...
#                 continue  # skip the entire object, don't emit a partial sequence
...
#     dataset = Pix2SeqDataset(JSON_DIR, IMG_DIR)
#     dataloader = DataLoader(dataset, batch_size=BATCH_SIZE, shuffle=True)

#     # Max_seq_len değerini model içine geçiriyoruz
#     model = Pix2SeqModel(vocab_size=VOCAB_SIZE, max_seq_len=dataset.max_seq_len).to(device)
#     criterion = nn.CrossEntropyLoss(ignore_index=PAD_TOKEN)
#     #optimizer = torch.optim.Adam(model.parameters(), lr=LEARNING_RATE)

#     optimizer = torch.optim.AdamW([
#     {'params': model.encoder.parameters(), 'lr': 1e-5}, # ResNet için çok daha düşük LR
#     {'params': model.enc_proj.parameters()},
#     {'params': model.pos_emb},
#     {'params': model.embedding.parameters()},
#     {'params': model.seq_pos_encoding.parameters()},
...
#     scheduler = OneCycleLR(optimizer, max_lr=3e-4, total_steps=total_steps, pct_start=0.05)
...
#             # Girdi: Son token hariç, Hedef: İlk token (BOS) hariç
...
#     print("Eğitim tamamlandı ve model kaydedildi: pix2seq_model_v2.pth")
