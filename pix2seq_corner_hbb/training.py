...
class Pix2SeqDataset(Dataset):
    def __init__(self, json_dir, img_dir, max_objects=MAX_OBJECTS, img_size=IMG_SIZE,
                 transform=train_transform, json_files=None, use_mosaic=True,
                 randomize_order=True, noise_augmentation=NOISE_AUGMENTATION):
        ...
        # Her __getitem__ çağrısında objelerin sequence içindeki sırası karıştırılır
        # (Pix2Seq makalesindeki gibi) — bu, YALNIZCA hangi objenin kaçıncı sırada
        # yazılacağını etkiler. Her objenin kendi 4 köşesinin sırası (canonical
        # etiketleme) buna dokunulmadan aynen korunur, bkz. __getitem__.
    ...
    def __getitem__(self, idx):
        ...
        # ── Mosaic (30% chance, sadece bu dataset'in kendi split'i içinden) ─────
        if self.use_mosaic and self.transform is not None and len(self.json_files) >= 4 and random.random() < 0.3:
        ...
        # Noise-object augmentation: gerçek objelerin yanına MAX_OBJECTS'e
        ...
        # Sadece objelerin sequence İÇİNDEKİ SIRASINI karıştırıyoruz (Pix2Seq
        # makalesindeki random-order augmentation) — hangi objenin köşeleri
        # olduğu ya da o köşelerin sırası değişmiyor, sadece "önce hangi obje
        # yazılsın" rastgeleleşiyor. Val'de bu kapalı (deterministik val loss).
        if self.randomize_order:
            random.shuffle(objects)

        sequence = [BOS_TOKEN]

        for class_id, shape_points in objects:
...
class Pix2SeqModel(nn.Module):
    def __init__(self, vocab_size, hidden_dim=256, nheads=8, num_layers=4, max_seq_len=200,
        ...
        # gibi). ResNet50 zaten derin olduğu ve dataset küçük/tek-domain olduğu
        # için 2 katman ile başlıyoruz — istenirse num_encoder_layers ile artırılabilir.
        encoder_layer = nn.TransformerEncoderLayer(d_model=hidden_dim, nhead=nheads, batch_first=True, dropout=0.1)
...
if __name__ == "__main__":
    ...
    train_dataset = Pix2SeqDataset(JSON_DIR, IMG_DIR, transform=train_transform,
                                    json_files=train_files, use_mosaic=True,
                                    randomize_order=True, noise_augmentation=NOISE_AUGMENTATION)
    val_dataset   = Pix2SeqDataset(JSON_DIR, IMG_DIR, transform=val_transform,
                                    json_files=val_files, use_mosaic=False,
                                    randomize_order=False, noise_augmentation=False)
    ...
    optimizer = torch.optim.AdamW([
        ...
        {'params': model.img_encoder.parameters(),     'lr': LEARNING_RATE},
    ...
    for epoch in epoch_bar:
        ...
        if avg_val_loss < best_val_loss:
            ...
            save_checkpoint(model, "pix2seq_best.pth")
