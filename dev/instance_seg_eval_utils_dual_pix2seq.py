...
MASK_EVAL_SIZE = 256  # Downsampled mask resolution to keep validation memory footprint low[cite: 1]
...
def parse_ground_truth_targets(mask_targets, num_valid_objs):
    """
    Extracts GT boxes, polygons, and labels directly from the [MAX_OBJECTS, SEQ_LEN] target tensors.
    """
    gt_boxes, gt_polys, gt_labels = [], [], []

    for obj_idx in range(num_valid_objs.item()):
        seq = mask_targets[obj_idx].tolist()
        if EOS_TOKEN in seq:
            seq = seq[:seq.index(EOS_TOKEN)]
        seq = [t for t in seq if t not in (BOS_TOKEN, PAD_TOKEN)]

        if len(seq) < 5 + 2 * NUM_POLY_PTS:
            continue
...
@torch.no_grad()
def evaluate_dual(model, loader, device=None):
    """
    ...
    for images, _, mask_targets, num_objs in loader:
        ...
        for b in range(images.size(0)):
            ...
            g_boxes, g_polys, g_labels = parse_ground_truth_targets(mask_targets[b], num_objs[b])
